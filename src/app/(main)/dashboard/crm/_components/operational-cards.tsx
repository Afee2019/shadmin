  ...
  return (
    <div className="grid grid-cols-1 gap-4 *:data-[slot=card]:shadow-xs sm:grid-cols-2 xl:grid-cols-3">
      <Card>
        ...
        <CardFooter>
          <p className="text-muted-foreground text-xs">线索数量较上月增长 18.2%</p>
        </CardFooter>
      </Card>
      ...
      <Card>
        ...
        <CardFooter>
          <div className="text-muted-foreground flex justify-between gap-1 text-xs">
            <span>追踪 {regionSalesData.length} 个地区</span>
            <span>•</span>
            <span>{regionSalesData.filter((r) => r.isPositive).length} 个地区增长</span>
          </div>
        </CardFooter>
      </Card>
